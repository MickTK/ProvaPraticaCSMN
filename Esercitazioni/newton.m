...
% di Newton
% @param funzione, derivata prima della funzione, valore iniziale, tolleranza, iterazioni massime
...
fd = fund(x0);       % Valore della derivata prima calcolata in x0
if (abs(fd) < 1e-10) % Se il valore fd è molto piccolo
    error('La derivata si annulla in x0.'); % Stampa a video un messaggio di errore
end

% Prima iterazione
k = 1;                % Numero iterazione
x_new = x0 - (f0/fd); % Valore di x(k+1)
...
while (abs(x_new - x0) > tol*max(1,abs(x_new))) && (k < kmax)
    ...
    fd = fund(x0);        % Calcola il nuovo denominatore
    x_new = x0 - (f0/fd); % Calcola il nuovo x(k+1) 
end
