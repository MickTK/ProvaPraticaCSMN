...
f = (D - E) \ b;          % Vettore f
rho = max(abs(eig(Bgs))); % Raggio spettrale di Bgs
x_old = x0;               % x^(k)
x_new = Bgs*x_old + f;    % x^(k+1)
